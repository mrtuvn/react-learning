import React from "react";

const CartItem = ({ ItemProps }) => {
  return (
    <div className="cardItem">
      <div className="cardItem_left">
        <div className="cardItem_image">
          <img alt="product image" src={ItemProps.item.image} />
        </div>
      </div>
      <div className="cardItem_right">
        <div className="cardItem_name">{ItemProps.item.name}</div>
        <div className="cardItem_price">${ItemProps.item.price}</div>
        <div className="cartItem_actions">
          <div className="cartItem_count">
            <div className="cartItem_button">-</div>
            <div className="cartItem_number">{ItemProps.item.quantity}</div>
            <div className="cartItem_button">+</div>
          </div>
          <div className="carItem_remove">
            <img
              onClick={ItemProps.handleRemove}
              alt=""
              src="data:image/png;base64,"
            />
          </div>
        </div>
      </div>
    </div>
  );
};

export default CartItem;
